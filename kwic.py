...
def _kwic_rows(pattern, it, k):
    for i,t in it:
        for m in re.finditer(pattern, t, re.I):
            s, e = m.span()
            yield f'''<tr><td>{i}</td>
                      <td style="text-align:right;white-space:nowrap">{t[max(0,s-k):s]:>{k}}</td>
                      <td style="text-align:center;white-space:nowrap">{t[s:e]}</td>
                      <td style="text-align:left;white-space:nowrap">{t[e:e+k]:{k}}</td></tr>'''